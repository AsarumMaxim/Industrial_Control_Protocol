...
当接收方收到报文时，会对整个报文（包括CRC校验码）使用相同的CRC计算流程。如果报文未被篡改，计算结果应为`0x0000`（考虑到了CRC码的加入和计算规则）。如果结果不是`0x0000`，则表明报文在传输过程中可能遭到了篡改或出现了错误。
...
**事务标识符**、**协议标识符**、**长度字段**、**单元标识符**四部分即为MBAP头。
